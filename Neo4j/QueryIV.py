...
beginning = time.time()
res = graph.run("""
    MATCH (l:Lawyers {id: '3'})-[:IS_INVOLVED]->(Claims)<-[:HAS_CLAIM]-(c)-[:HAS_ADDRESS]->(h)
    WHERE h.address CONTAINS '5'
    RETURN c.name, c.surname, h.address
""")
for re in res:
    print(re)
...
with open('C:\\Users\\39377\\Desktop\\ProgettoDbNoSql\\Neo4j\\RisultatiNeo4j\\Query100%\\QueryIV100%.csv', 'w') as results:
