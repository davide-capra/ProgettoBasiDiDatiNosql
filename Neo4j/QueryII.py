...
beginning = time.time()
res = graph.run("""MATCH (l:Lawyers {id: '3'})-[:IS_INVOLVED]->(Claims)<-[:IS_EVALUATOR]-(e) RETURN e.name, e.surname""")
for re in res:
    print(re)
...
for i in range(1, 31):
    begquery = time.time()
    res = graph.run("""MATCH (l:Lawyers {id: '3'})-[:IS_INVOLVED]->(Claims)<-[:IS_EVALUATOR]-(e)
...
with open('C:\\Users\\39377\\Desktop\\ProgettoDbNoSql\\Neo4j\\RisultatiNeo4j\\Query100%\\QueryII100%.csv', 'w') as results:
